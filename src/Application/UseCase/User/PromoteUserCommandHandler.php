...
final class PromoteUserCommandHandler
{
     ...
     * @return void
     */
    public function handle(UserType $userType ,PromoteUserCommand $promote_user): void
    {
        try {
            /**
         ...
         */
        foreach ($promote_user->rolesNew as $role) {
            $user->addRole($role);
        }
         ...
         */
        $this->eventBus->dispatch(new PromoteUserEvent($user,$promote_user->rolesNew));
    }
}
