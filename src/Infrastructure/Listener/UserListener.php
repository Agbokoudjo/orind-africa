<?php

declare(strict_types=1);

/*
 * This file is part of the project by AGBOKOUDJO Franck.
 *
 * (c) AGBOKOUDJO Franck <internationaleswebservices@gmail.com>
 * Phone: +229 01 67 25 18 86
 * LinkedIn: https://www.linkedin.com/in/internationales-web-apps-services-120520193/
 * Github: https://github.com/Agbokoudjo/
 * Company: INTERNATIONALES WEB APPS & SERVICES
 *
 * For more information, please feel free to contact the author.
 */

namespace App\Infrastructure\Listener;

use Doctrine\ORM\Events;
use App\Domain\User\BaseUserInterface;
use Doctrine\Persistence\Event\LifecycleEventArgs;
use App\Domain\User\Service\UserAccountListenerInterface;
use Doctrine\Bundle\DoctrineBundle\Attribute\AsDoctrineListener;

/**
 * @author AGBOKOUDJO Franck <internationaleswebservices@gmail.com>
 * @package <https://github.com/Agbokoudjo/>
 */
#[AsDoctrineListener(event: Events::prePersist, priority: 500)]
#[AsDoctrineListener(event: Events::preUpdate, priority: 500)]
final class UserListener 
{
    public function __construct(private UserAccountListenerInterface $userEventListener) {}

    public function prePersist(LifecycleEventArgs $args): void
    {
        $entity = $args->getObject();
        if (!$entity instanceof BaseUserInterface) return;
        $this->userEventListener->prePersist($entity);
    }

    public function preUpdate(LifecycleEventArgs $args): void
    {
        $entity = $args->getObject();
        if (!$entity instanceof BaseUserInterface) return;
        $this->userEventListener->preUpdate($entity);
    }
}
